...
const registerUser = async(req, res)=>{
    ...
    try{
        ...
        user = new User({email, password:(bcrypt.hashSync(password, salt)), tokenConfirm: randomId(largo, patrón)})
...
const loginUser = async(req, res)=>{
    ...
    try{
        ...
        let validacionPassword = await bcrypt.compareSync(req.body.password, user.password);
...
const guardarNuevaPass = async(req, res)=>{
    ...
    try{
        ...
        await user.updateOne({password:(bcrypt.hashSync(password, salt))})
